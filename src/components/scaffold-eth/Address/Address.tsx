...
export const Address = ({
  ...
  useEffect(() => {
    if (isMounted())
      setBlockExplorerUrl(
        `${caipNetwork?.blockExplorers?.default.url}/address/${checkSumAddress}`
      );
  }, [caipNetwork?.blockExplorers?.default.url, checkSumAddress, isMounted]);
          ...
          (isEnsNameLoading ? (
            ...
            <span className={`ml-1.5 ${textSizeMap[ensSize]} font-bold`}>
              <AddressLinkWrapper
                ...
                blockExplorerAddressLink={blockExplorerUrl}
              >
                {ens}
              </AddressLinkWrapper>
            </span>
